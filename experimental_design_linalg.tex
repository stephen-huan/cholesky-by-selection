...
The Vecchia approximation \cite{vecchia1988estimation} decomposes
a Gaussian distribution into the product of univariate densities,
each of which depends on a subset of the previous variables.
This amounts to a sparse approximation of the inverse-Cholesky
factor of the covariance matrix \cite{katzfuss2021general}.
Other works derive the same approximation as the optimal
Cholesky factor for a given ordering and sparsity pattern:
Kaporin minimizes the \( K \)-condition number \cite{kaporin1990alternative},
Yeremin et al. the Frobenius norm subject to a diagonal scaling constraint
\cite{yeremin2000factorized}, and Sch{\"a}fer et al. the Kullback-Leibler (KL)
divergence \cite{schafer2021sparse}.
