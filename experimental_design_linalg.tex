...
Alternative approaches are based on the fast Fourier
transform \cite{graham2018analysis, stein2002fast}
or random feature maps \cite{rahimi2007random}.

\paragraph{Vecchia approximation, Kaporin's factorization, and KL minimization}

Vecchia approximates the likelihood function of a Gaussian distribution by decomposing it into a product of univariate conditional densities,
each of which depends on a subset of the previous variables \cite{vecchia1988estimation}.
Independently from Vecchia, Kaporin derived a closed-form expression of the approximate inverse Cholesky preconditioner of a p.s.d. matrix that minimizes, subject to a sparsity constraint, the $K$-condition number of the preconditioned system \cite{kaporin1990alternative}.
Referring to this approximation as ``factorized sparse approximate inverse (FSAI)'', Yeremin et al. show that Kaporin's inverse Cholesky factor also minimizes a Frobenius-norm error metric, subject to a diagonal scaling constraint. 
Vecchia's likelihood approximation was observed to implicitly compute a sparse approximate inverse Cholesky factor of the covariance matrix \cite{katzfuss2021general}.
The closed-form expression of this inverse Cholesky factor coincides with that derived by Kaporin. 
Independently from the above works, Sch{\"a}fer et al. compute sparse inverse Cholesky factors that are optimal in Kullback-Leibler (KL) divergence \cite{schafer2021sparse}, again recovering the formula derived by Kaporin.  
...
orderings like the maximum-minimum ordering \cite{guinness2018permutation} also
used by \cite{schafer2020compression, schafer2021sparse, katzfuss2021general,
kang2021correlationbased, katzfuss2022scalable}.
Cholesky factorization in the maximum-minimum ordering
is closely related to numerical homogenization and
operator-adapted wavelets \cite{owhadi2019operatoradapted}.
For covariance functions that are Green's functions of
elliptic PDEs, this allows proving the exponential accuracy
of the resulting approximation \cite{schafer2020compression}.
...
Methods exploiting and quantifying the screening effect achieve
state-of-the-art \( \epsilon \)-accuracy in time complexity \( \BigO(N
...
and Simulation) and the Department of Energy under the MMICCs SEA-CROGS award.
