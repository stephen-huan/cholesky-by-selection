...
which have become relatively popular \cite{schafer2020compression,
schafer2021sparse, kang2021correlationbased}.
...
However, selecting by distance alone ignores the conditional
...
%     \( \mu \), conditional on the \( k \) selected points, and the \(
%     \mu - 2 \std \) to \( \mu + 2 \std \) confidence interval is shaded
%     for the conditional variance \( \var \).}
%   \label{fig:select}
% \end{figure}
...
In the literature, factorized sparse approximate inverse (FSAI)
preconditioners are computed by \cite{kaporin1990alternative,
yeremin2000factorized}, while sparse approximate Cholesky factors
of the precision are computed by \cite{schafer2021sparse}.
Both of these approaches can be viewed through the lens of an
early approach for fast Gaussian process regression, the Vecchia
approximation \cite{vecchia1988estimation, katzfuss2021general}.
...
\begin{align}
  ...
  L &\defeq \argmax_{\hat{L} \in \SpSet} \loss(\hat{L})
\end{align}
where \( \SpSet \defeq \{ M \in \Reals^{N \times N} :
M_{ij} \neq 0 \Rightarrow (i, j) \in S \} \) is the space
of matrices satisfying the sparsity pattern \( S \).
Multiple functionals in the literature actually converge to the same
closed-form expression for the entries of the resulting Cholesky factor
\( L \): Minimizers of the Kaporin condition number \( (\trace(L \CM
L^{\top})/N)^N/\det(L \CM L^{\top}) \) \cite{kaporin1990alternative},
minimizes of \( \norm{\Id - L \chol(\CM)}_{\FRO} \) additionally subject to
the constraint \( \diag(L \CM L^{\top}) = 1 \) \cite{yeremin2000factorized},
and minimizers of the KL divergence \( \KL{\N(\vec{0},
\CM)} {\N(\vec{0}, (L L^{\top})^{-1})} \) \cite{schafer2021sparse}.
This shared closed-form expression for the entries of the resulting
factor can be shown to be equivalent to the formula used to compute the
Vecchia approximation for Gaussian processes \cite{vecchia1988estimation}.
In addition, using the KL divergence to compute Cholesky factors can
be seen as a linear special case of computing sparse lower triangular
maps in \cite{marzouk2016introduction, katzfuss2022scalable}.

The primary difference between the numerical linear algebra viewpoints
(Kaporin conditional number or Frobenius norm) and the statistical viewpoints
(KL divergence, transport maps) is whether the covariance is factored, \( L
\chol(\CM) \approx \Id \), or the precision, \( L L^{\top} \approx \CM^{-1} \).
...
\Stodo{reverse maximin ordering \cite{guinness2018permutation,
schafer2021sparse, schafer2020compression}}
...
Writing \cref{eq:L_obj} in the form of \cref{eq:kl} and optimizing for \(
L \) yields the following closed-form expression for the \( i \)th column
of \( L \) with sparsity pattern \( s_i \), reproduced from Theorem 2.1 of
...
\begin{align}
  ...
  (\vec{e}_1^{\top} \CM_{s_i, s_i}^{-1} \vec{e}_1)^{-1}
         &= \CM_{i, i \mid s_i - \{ i \}}
\end{align}
So minimizing the KL divergence of the sparse Cholesky factor \( L \) is
equivalent to independently minimizing over each column the variance of the
\( i \)th variable, conditional on the entries \( s_i \) (excluding itself).
A decomposition of the KL divergence into
independent regression problems was also observed in
\cite{katzfuss2022scalable} for lower triangular transport maps.
From a Gaussian process regression perspective, we wish to carefully select the
most effective points \( s_i \) out of the possible candidates \( i + 1, i +
2, \dotsc, N \) that most reduce predictive error at the point of interest \(
\vec{x}_i \).

In the next section, we develop such a greedy selection
algorithm for directed inference in Gaussian processes.
We will return to sparse Cholesky factorization and apply this algorithm
for sparsity selection of Cholesky factors in \cref{sec:chol_select}.
