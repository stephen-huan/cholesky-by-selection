...
\begin{align}
  ...
  \end{pmatrix}
\end{align}
Here the conditional expectation \cref{eq:cond_mean} corresponds
to \( \textcolor{darkorange}{\CM_{2, 1} \CM_{1, 1}^{-1}} \)
and the conditional covariance \cref{eq:cond_cov} corresponds to
\(
...
covariance matrix of the target and training points, whose columns consist
of the target points and previously selected points sorted in reverse order
...
\end{align}
where unlike \cref{eq:chol_partial}, we use a lower-triangular factor \( L
\) such that \( L^{\top} L = \CM \) for simplicity: if \( U = P^{\Reverse}
\chol(P^{\Reverse} \CM P^{\Reverse}) P^{\Reverse} \) where \( P^{\Reverse}
\) is the order-reversing permutation, then \( U \) is an upper-triangular
factor satisfying \( U U^{\top} = \CM \) so \( L = U^{\top} \) satisfies \(
L^{\top} L = \CM \).

To account for this insertion, the first \( p \) columns \( L_{:, :p} \)
are unchanged, the newly inserted column \( \vec{u} \) as a variance-scaled
conditional covariance of the form \cref{eq:cond_select} can be computed
as usual with left-looking (see \cref{alg:chol_update}), and the remaining
columns \( {L'}_{:, p + 1:} \) are the Cholesky factor of the covariance
...
\begin{figure}[t]
  ...
  \caption{
    ...
    the \textcolor{seagreen}{sparsity pattern} \( s_i \).
...
Using the single-target selection algorithm in \cref{subsec:greedy_select}
...
each candidate conditions every point in the group and so the multiple-target
...
Of course, the multiple-target algorithm can still be applied, but it
...
If the average group size is \( m \) points, for both the multiple-target
...
The multiple-target algorithm already computes the difference in log
determinant after selecting the candidate so no modification must be made.
...
Using the same flipping trick as \cref{subsec:partial_select}, let \(
U = P^{\Reverse} \chol(P^{\Reverse} \CM_{s_{\tilde{i}}, s_{\tilde{i}}}
P^{\Reverse}) P^{\Reverse} \) where \( P^{\Reverse} \) is the order-reversing
permutation; \( U \) is upper-triangular and satisfies \( U U^{\top}
= \CM_{s_{\tilde{i}}, s_{\tilde{i}}} \) so \( U^{-\top} U^{-1} =
\CM_{s_{\tilde{i}}, s_{\tilde{i}}}^{-1} = Q \) and we see that \( L = U^{-\top}
\) is a lower-triangular Cholesky factor satisfying \( L L^{\top} = Q \).
