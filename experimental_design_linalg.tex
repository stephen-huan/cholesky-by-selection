...
a point can only include points before it in the ordering.
...
the column indices \( i_1 \prec i_2 \prec \dotsb \prec i_m \).
...
\begin{align}
  ...
  \sum_{i \in \tilde{i}} \log(\CM_{i \mid s_i \setminus \{ i \} }) &=
    \log(\CM_{i_m \mid \tilde{s}}) +
    \log(\CM_{i_{m - 1} \mid \tilde{s} \cup \{ i_m \}}) + \dotsb +
    \log(\CM_{i_1 \mid \tilde{s} \cup \tilde{i}}) \\
  \shortintertext{where we write \( \CM_j \defeq \CM_{j, j} \).
    Combining the first two terms by the chain rule \cref{eq:det_chain},}
  &= \logdet(\CM_{\{ i_m, i_{m - 1} \} \mid \tilde{s}}) +
     \log(\CM_{i_{m - 2} \mid \tilde{s} \cup \{ i_m, i_{m - 1} \}}) + \dotsb +
     \log(\CM_{i_1 \mid \tilde{s} \cup \tilde{i}}) \\
...
\end{align}
So the suitable generalization of the conditional variance in
\cref{eq:obj_chol} to aggregated columns is the log determinant of the
covariance matrix of the columns, conditional on the selected entries.
...
We will apply this algorithm for sparsity selection
...
The second strategy is to take advantage of
the quotient rule of Schur complementation.
...
of unknown data can be done directly according to \cref{eq:cond_mean} and
\cref{eq:cond_cov} in time \( \BigO(s^3) \) using \cref{alg:infer_select}.
