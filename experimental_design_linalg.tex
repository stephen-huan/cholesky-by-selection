...
\end{align}
So the generalization of the posterior variance in \cref{eq:obj_chol}
to aggregated columns is the log determinant of their covariance
matrix, conditional on (well-behaved) selected entries.
...
We apply this algorithm for sparsity selection of
sparse Cholesky factors in \cref{sec:chol_select}.
...
Both approaches have a time complexity of \( \BigO(N s^2 + N m^2
+ m^3) \) to select \( s \) points out of \( N \) candidates
for \( m \) targets, again differing in space complexity.
...
The Cholesky algorithm requires \( \BigO(N s + N m + m^2) \)
memory to store two \( (N + m) \times (m + s) \) sized partial
Cholesky factors for the joint covariance matrix \( \CM \).
...
precision algorithm but is again preferred for simplicity and performance.
...
time complexity for a single column (see \cref{app:L_mult} or Algorithm 3.2
of \cite{schafer2021sparse}).
...
Inheriting the downside of the multiple-target selection algorithm,
the aggregated factor is less efficient at reducing the KL divergence
per nonzero since the sparsity pattern is shared between all columns
in the group, not tailored to any particular column.
...
sparse Cholesky factor \( L \) from its covariance matrix \( \CM = L L^{\top}
\).
...
value 10 to ensure \( \CM \) is well-conditioned.
The selection algorithm is given \( s \) and either \( \CM
\) or the precision matrix \( \CM^{-1} \) depending on
which results in higher accuracy in reconstructing \( L \).
For a recovered sparsity pattern \( X \) and ground truth \( Y \) we report \(
\frac{\card{X \cap Y}}{\card{X \cup Y}} \), the intersection over union (IOU).
Using the KL divergence \cref{eq:L_obj} of the Cholesky factor
obeying the recovered sparsity seems mostly equivalent to IOU.
...
\( \N(0, \var) \) is symmetrically added to each entry of \( \CM
\) (\( \CM_{ij} \) receives the same noise as \( \CM_{ji} \)).
...
At high enough levels of noise \( \CM \) can lose
...
\begin{figure}[t]
  ...
  \caption{
    We attempt to recover a sparse Cholesky factor \( L \)
    from its covariance matrix \( \Theta = L L^{\top} \).
    ...
    All methods achieve their best accuracy given \( \CM \) except for
    C\( k \)-NN which is given the precision matrix \( \CM^{-1} \).
...
\begin{figure}[t]
  ...
  \caption{
    ...
    \) to the measurements \( \Theta \) entrywise.
...
We sample \( N \) points uniformly at random from the unit cube
...
time increases linearly with the number of points for all methods.
Conditional methods are more accurate than their unconditional counterparts and
aggregated variants are both more accurate and faster than their non-aggregated
counterparts; however the aggregated factors are denser, which may impact their
...
Part of that improvement is from keeping the number of nonzeros per column
constant as \( k \)-NN is also significantly more accurate than the baseline;
\cite{kang2021correlationbased} also observes significantly better accuracy
from \( k \)-NN for irregularly spaced points.
The conditional methods achieve better accuracy per unit
time cost than their unconditional counterparts, but
the simple method of \( k \)-NN remains hard to beat.

Finally, we experiment with increasing the size of the candidate set \(
\rho_s \) \emph{without} increasing the density of the factor \( \rho \).
...
\begin{figure}[t]
  ...
  \caption{
    We vary the size of the candidate set, \( \rho_s \),
    with the density of the factor fixed at \( \rho = 4 \)
    and the number of points fixed at \( N = 2^{16} \).
    The left panel shows KL divergence with an increasing number of candidates
    considered and the right panel shows the corresponding increases in time.
  }
  \label{fig:chol_s}
\end{figure}
...
We sample \( 2^{16} \) points uniformly at random from the unit
cube \( [0, 1]^3 \) and randomly partition them into 90\% training
points (\( N = 58982 \)) and 10\% prediction points (\( m = 6554 \)).
We use a Mat{\'e}rn kernel with smoothness \( \nu =
\frac{5}{2} \) and length scale \( \ell = 1 \) and draw \(
10^3 \) realizations from the resulting Gaussian process.
...
nearly identical to Cholesky factorization (\cref{subsec:chol_exp}).
We find that coverage is extremely accurate for all
methods (within \( 0.1\% \) for \( \rho \geq 2 \)).
Finally, the RMSE is shown in \cref{fig:gp_rho}.
Unlike the KL divergence, aggregated variants are nearly
indistinguishable in RMSE from their non-aggregated counterparts.
...
like \( \nu = \frac{5}{2} \) led to extremely poor condition and
...
\cite{gramacy2014local} and the follow-up work \cite{gramacy2015speeding} apply
ALC to directed Gaussian process inference, yielding an algorithm equivalent
to our \cref{alg:select_prec} (described in \cref{subsec:greedy_select}) for a
...
We generalize ALC to multiple prediction points through their posterior
log determinant \cref{eq:greedy_mult} and provide an explicit
\cref{alg:select_mult_chol} described in \cref{subsec:mult_select}.
In addition, integration of the algorithm into Cholesky factorization
(\cref{sec:chol_select}) provides a global approximation of the
Gaussian process (\cref{subsec:gp_exp}).
...
We wish to compute the KL divergence between \( \CM \) and the optimal sparse
Cholesky factor \( L \) computed from the closed-form \cref{eq:L_col}.
...
\cref{subsubsec:aggregated}, we have indices \( \tilde{i} = \{i_1,
...
In the partial aggregated Cholesky factorization setting of
\cref{subsubsec:partial}, we have a collection of random variables \( \vec{y} =
[y_1, \dotsc, y_m]^{\top} \) corresponding to the ordered column indices \( i_1
\succ \dotsb \succ i_m \) grouped in \( \tilde{i} = \{ i_1, \dotsc, i_m \} \).
We select an index \( k \) that conditions all but the first \( p \) variables;
we denote the updated variables as \( \vec{y}_{\parallel k} \defeq [y_1,
\dotsc, y_p, y_{p + 1 \mid k}, \dotsc, y_{m \mid k}]^{\top} \) and wish to
compute their covariance \( \Cov[\vec{y}_{\parallel k}] \).
The original variables \( \vec{y} \) have joint density multivariate
Gaussian, \( \vec{y} \sim \N(\vec{0}, \CM) \) for some covariance
matrix \( \CM \), and the fully conditional variables \( \vec{y}_{\mid
k} \) have posterior distribution according to \cref{eq:cond_cov}, \(
\vec{y}_{\mid k} \sim \N(\vec{\mean}, \CM - \CM_{:, k} \CM_{k, k}^{-1}
\CM_{k, :}) \) for some
posterior mean \( \vec{\mean} \).
For unconditioned \( y_i \) and \( y_j \), their covariance is simply
\( \CM_{i, j} \); similarly, for conditioned \( y_{i \mid k} \) and
\( y_{j \mid k} \), their covariance is \( \CM_{i, j \mid k} \).
...
We can then view \( \vec{y} \) as \( L \vec{z} \) for \( \vec{z} \sim
\N(\vec{0}, \Id) \) and \( \vec{y}_{\mid k} = L' \vec{z} + \vec{\mean} \).
...
Given the log determinant of the covariance matrix of the prediction points
conditional on the selected entries, \( \logdet(\CM_{\Pred, \Pred \mid \I})
\), we wish to compute the log determinant after we add an index \( k \) to
\( \I \), that is, to compute \( \logdet(\CM_{\Pred, \Pred \mid \I, k}) \).
...
The approach storing two partial Cholesky factors is considerably
...
Over \( s \) points selected the total time complexity
after simplification is \( \BigO(N s^2 + N m^2 + m^3) \).
