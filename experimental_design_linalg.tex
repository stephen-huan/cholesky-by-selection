...
scale \( \ell = 1 \), drawing \( 10^3 \) realizations from the resulting
Gaussian process for the target variable, ignoring the original objective
to ensure that the target is exactly Gaussian.
...
is low-dimensional (often 2 or 3 in spatial statistics).
...
FS thanks Joel Tropp for fruitful discussions on JT's  proposal to use matching pursuit algorithms for selecting sparsity patterns of Cholesky factors.
