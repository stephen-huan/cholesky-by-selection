...
\Stodo{todo}
If the objective satisfies the property of submodularity, then
it is guaranteed that the greedy algorithm produces an objective
within a \( 1 - \frac{1}{e} \) factor of the optimal objective.
Unfortunately the information gain objective is not
submodular in general, see \cref{app:submodular}.
However, it is possible to empirically observe for a particular
point set and choice of kernel function whether it is submodular.
We note that one-dimensional points with a Mat{\'e}rn kernel
function seems to be submodular, but not for any higher dimension.
...
All experiments were run on the Partnership for an Advanced
Computing Environment (PACE) Phoenix cluster at the Georgia
Institute of Technology, with 8 cores of a Intel Xeon
Gold 6226 CPU @ 2.70GHz and 22 GB of RAM per core.
The code is written in Python (using standard scientific
libraries like numpy, scipy, scikit-learn, matplotlib) as well
as Cython which provides transpilation of Python code into C.
Cython allows Python code to access native C
interfaces to the BLAS and Intel MKL libraries.
...
smoothness \( \nu = \frac{3}{2} \) and length scale \( \ell = 2^{10} \). Finally, we
...
from \( \N(0, \var) \) is added to each entry of \( \CM
...
\paragraph{Sparse Gaussian process regression}

\paragraph{Local approximate Gaussian process (laGP)}

\paragraph{Orthogonal matching pursuit (OMP)}

\cite{bartels2022adaptive}

In the context of Gaussian process regression, this selection scheme falls
into the categories of subset of data (SoD) \cite{chalupka2012framework,
quinonero-candela2005unifying}, transductive naive-local-expert (NLE)
\cite{liu2020when}, and active set selection, particularly greedy
selection that maximizes an information-theoretic quantity of interest
\cite{smola2000sparse, herbrich2002fast, seeger2003fast}.

Our objective for point selection was first described in
\cite{cohn1996neural} for optimal experimental design.

% covariance matrix approximation / likelihood
% approximation / local methods / active set
% naive-local-expert
%
% \cite{herbrich2002fast} algorithm 1 similar
%
...
% (never mind, smola \cite{smola2000sparse} is directed --- or maybe not)
%
% \Stodo{search for directed inference Gaussian process regression}
%
% omp: feature space versus covariance
%
% engineering for multiple points / nonadjacent case (conditioning structure)
%
% summary papers liu2020when, chalupka2012framework,
% quinonero-candela2005unifying,
%
% wada2013gaussian
%
% cohn1996neural
% gramacy2014local follow up work gramacy2015speeding
% equation 5 (conditional variance), equation 8 (objective)
% (SM\S4 SM.22)
% partition inverse equations (SM\S1)
% section 3.3
%
% section 4 naive way to do multiple points without multiple point selection /
% near linear joint Gaussian process
% but embarrassingly parallel like columns in cholesky factor
%
% subsampling within nn candidate set in follow up work
...
%
% Reduction to Katzfuss/sparse chol by kl if kth nearest neighbors
% / directed active set used instead of greedy selection
