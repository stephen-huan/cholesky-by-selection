...
\( j \) variables (recall that the indices are sorted in decreasing order,
...
k} \defeq y_1, \dotsc, y_j, y_{j + 1 \mid k}, \dotsc, y_{m \mid k} \)
...
\begin{align}
  ...
  \underbrace{\log(L_{1, 1}) + \dotsb + \log(L_{j, j})}_{\text{the same}} +
  \underbrace{
    \log({L'}_{j + 1, j + 1}) + \dotsb + \log({L'}_{m, m})
  }_{\text{conditioned}}
  \shortintertext{Comparing to the KL divergence \cref{eq:obj_chol}
    and recalling that \( k \) is added to \( s_i \) if \( i > j \),}
  \sum_{i = 1}^m \log \left ( \CM_{i, i \mid s_i \setminus \{ i \}} \right )
  &= \underbrace{
      \log \left ( \CM_{1, 1 \mid s_1 \setminus \{ 1 \}} \right ) + \dotsb +
      \log \left ( \CM_{j, j \mid s_j \setminus \{ j \}} \right )
     }_\text{the same} + \\
  \nonumber
  ...
  \underbrace{
    \log \left (
      \CM_{j + 1, j + 1 \mid s_{j + 1} \setminus \{ j + 1 \}}
    \right ) + \dotsb +
...
By the quotient rule, the order of conditioning
does not matter as long as the order is consistent.
...
both Cholesky factors in time \( (N + m)(m + s) \) which is
