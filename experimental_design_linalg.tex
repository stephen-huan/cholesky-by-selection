...
\)-NN). Given a training set \( X_\text{Tr} = \{ \bm{x}_1, \dots, \bm{x}_n \}
\) and corresponding labels \( \bm{y}_\text{Tr} = \{ y_1, \dots, y_n \} \),
the goal is to estimate the unknown label \( y_\text{Pr} \) of some unseen
prediction point \( \bm{x}_\text{Pr} \) Stated informally, the \( k \)-NN
approach is to select the \( k \) points in \( X_\text{Tr} \) \emph{most
informative} about \( \bm{x}_\text{Pr} \) and combine their results.

\begin{algorithm}
  \caption{Idealized \( k \)-NN regression}
  Given \( (X_\text{Tr}, \bm{y}_\text{Tr}) = 
          \{ (\bm{x}_1, y_1), \dots, (\bm{x}_n, y_n) \} \) 
  and \( \bm{x}_\text{Pr} \)
  \begin{enumerate}
    \item Select the \( k \) points in \( X_\text{Tr} \)
      most informative about \( \bm{x}_\text{Pr} \)
    \item Combine the labels of the selected points to generate a prediction
  \end{enumerate}
\end{algorithm}

One specific approach is intuitively, points close to \( \bm{x}_\text{Pr}
\) should be similar to it. So we select the \( k \) closest points in \(
X_\text{Tr} \) to \( \bm{x}_\text{Pr} \) and pool their labels (e.g., by
averaging).

\begin{algorithm}
  \caption{\( k \)-NN regression}
  \begin{enumerate}
    \item Select the \( k \) points \( \{ \bm{x}_{i_1}, \dots, \bm{x}_{i_k} \} 
      \subseteq X_\text{Tr} \) closest to \( \bm{x}_\text{Pr} \) 
    \item Compute \( \bm{y}_\text{Pr} \) by 
...
\begin{algorithm}
  ...
  Given kernel function \( K(\bm{x}, \bm{y}) \) 
  \begin{enumerate}
    \item Select the \( k \) points \( \{ \bm{x}_{i_1}, \dots, \bm{x}_{i_k}
      \} \subseteq X_\text{Tr} \) most similar to \( \bm{x}_\text{Pr} \)
    \item Compute \( \bm{y}_\text{Pr} \) by an average weighted by similarity
  \end{enumerate}
\end{algorithm}
...
point to \( \bm{x}_\text{Pr} \) has many duplicates in the training set. Then
...
\begin{algorithm}
  ...
  \begin{enumerate}
    \item Select the \( k \) points \( \{ \bm{x}_{i_1}, \dots, \bm{x}_{i_k} \} 
      ...
      \( \bm{x}_\text{Pr} \) \\ \textcolor{lightblue}{after conditioning on
      all points selected beforehand}
    \item Compute \( \bm{y}_\text{Pr} \) by an average weighted by
...
\todo{mention sensor placement/spatial statistics literature}
...
Gaussian process is specified by a \emph{mean function} \( \mu(\bm{x})
\) and \emph{covariance function} or \emph{kernel function} \( K(\bm{x},
\bm{y}) \). For any finite set of points \( X = \{ \bm{x}_1, \dots, \bm{x}_n
\} \), \( f(X) \sim \mathcal{N}(\bm{\mu}, \Theta) \), where \( \bm{\mu}_i
= \mu(\bm{x}_i) \) and \( \Theta_{ij} = K(\bm{x}_i, \bm{x}_j) \).

In order to compute a prediction at \( \bm{x}_\text{Pr} \), we can
simply condition the desired prediction \( \bm{y}_\text{Pr} \) on
the observed outputs and compute the conditional expectation. We
can also find the conditional variance, which will quantify the
uncertainty of our prediction. If we block our covariance matrix
\( \Theta = 
  \begin{pmatrix} 
...
efficiently compute these entropies. Because the entropy of \( \bm{y}_\text{Pr}
...
\cref{eq:cond_mean}, it is unbiased because \( \E[\E[\bm{y}_\text{Pr} \mid
\bm{y}_\text{Tr}]] = \E[\bm{y}_\text{Pr}] \). Because it is unbiased, its
expected mean squared error is simply the conditional variance since \(
\E[(\bm{y}_\text{Pr} - \E[\bm{y}_\text{Pr} \mid \bm{y}_\text{Tr}])^2 \mid
\bm{y}_\text{Tr}] = \Var[\bm{y}_\text{Pr} \mid \bm{y}_\text{Tr}] \) where
the expectation is taken under conditioning because of the assumption that
\( \bm{y}_\text{Pr} \) is distributed according to the Gaussian process. So
...
\begin{align*}
  \textcolor{orange}{\entropy[\bm{y}_\text{Pr}]} &= 
    \textcolor{lightblue}{\entropy[\bm{y}_\text{Pr} \mid \bm{y}_\text{Tr}]} +
    \textcolor{rust}{\I[\bm{y}_\text{Pr};\bm{y}_\text{Tr}]} \\
  \textcolor{orange}{\Var[\bm{y}_\text{Pr}]} &= 
    \textcolor{lightblue}{\E[\Var[\bm{y}_\text{Pr} \mid \bm{y}_\text{Tr}]]} +
    \textcolor{rust}{     \Var[\E[\bm{y}_\text{Pr} \mid \bm{y}_\text{Tr}]]} 
\end{align*}
...
information term. Supposing \( \bm{y}_\text{Pr} \) was independent of \(
\bm{y}_\text{Tr} \), then the conditional expectation becomes simply the
...
\bm{y}_\text{Pr} \) and \( \bm{y}_\text{Tr} \), as the larger it is, the
more the prediction for \( \bm{y}_\text{Pr} \) (the conditional expectation)
depends on the observed results of \( \bm{y}_\text{Tr} \).

\subsection{A greedy approach}

We now consider how to efficiently minimize the conditional variance objective
...
already. Let the prediction point have index \( n \), the last index. For a
candidate index \( k \), \todo{elaborate on this argument}, because iterative
conditioning of the Gaussian process is Schur complementation and Cholesky
factorization is recursive Schur complementation, we see that the amount
selecting \( k \) will decrease the variance of \( \bm{y}_\text{Pr} \) is
\begin{align}
  \label{eq:gp_greedy}
...
We now consider how to efficiently deal with multiple prediction points.
\todo{justifying objective: mutual information leads to logdet of conditional
covariance, information geometry perspective of logdet volume of uncertainty,
scaling factor in probability density function for multivariate Gaussian}

\subsection{Near optimality by submodularity}

\section{Greedy selection for \emph{global} approximation by KL-minimization}

We have a symmetric, positive (semi-)definite kernel matrix \( \Theta \)
and wish to compute the \emph{Cholesky factorization} of \( \Theta \) into
a lower triangular factor \( L \) such that \( \Theta = L L^{\top} \).
\todo{justify importance/applications of Cholesky factorization}. This
can be done in \( \mathcal{O}(N^3) \) with standard algorithms, which is
often prohibitive. Instead, we want to do so in a \emph{sparse} manner. As
such, we can only get an approximate \( L \), \( \hat{L} \) belonging to
a pre-specified sparsity pattern \( S \) (a set of (row, column) indices
that are allowed to be nonzero). In order to measure the performance of
the estimator, we imagine both kernel matrices as the covariance matrices
of multivariate Gaussians with mean \( \bm{0} \). In order to compare the
resulting two distributions, we use the \emph{KL divergence} according to
...
\todo{write Schur complements as explicit conditioning}
\begin{align}
  \nonumber
...
\todo{be more explicit that the objectives themselves are identical,
not just the greedy objectives (from conditioning being Schur complementation
being taking the matrix, inverting, taking a submatrix, inverting)}
...
\subsection{Iterative conditioning as Schur complementation
as Cholesky factorization as Orthogonal Matching Pursuit}
