...
\begin{abstract}
  ...
  Previous work in constructing sparse approximate
...
Applying Gaussian process inference to sets of \( N \) data
...
factor of the covariance matrix \cite{Datta2016,katzfuss2021general}.
...
He found randomly ordering and a space-filling
...
To lower the computational cost, different authors have proposed heuristics to
analyze the sparsity set and identify opportunities to group similar degrees
of freedom into supernodes \cite{stein2004approximating, ferronato2015novel,
guinness2018permutation}.
...
Our method extends kernel-based selection \cite{wada2013gaussian,Katzfuss2020,
...
\begin{figure}[t]
  ...
  \caption{
    ...
    of a dense grid of \( 51 \times 51 \) \textcolor{silver}{candidates}
    for \(k = 265 \).
...
\begin{align}
  ...
  \shortintertext{
    ...
    in the ordering, the Vecchia approximation replaces
...
dense, but the approximate factors of their inverses can be
...
Vecchia approximation \cite{katzfuss2021general,Katzfuss2018,Cao2022} and Cholesky
...
\cref{eq:kl}, we obtain the objective as a function of the sparsity pattern (see \cref{app:kl_L} for details; importantly,
...
target point as well as to minimizing the expected mean squared error
...
To select
\( s \) points out of \( N \) candidates, both approaches have a time complexity of \( \BigO(N s^2) \) but differ in space complexity:
The precision takes \( \BigO(s^2) \) space, while the first
\( s \) columns of the Cholesky factor of \( \CM \) use
\( \BigO(N s) \) space.
Both algorithms use \( \BigO(N) \) space
to store the conditional (co)variances.
The precision algorithm uses less memory than the Cholesky algorithm, but the Cholesky algorithm is easier to implement and
...
the contribution from group \( \tilde{i} \) is
\begin{align}
  \label{eq:obj_mult}
  \sum_{i \in \tilde{i}}
    \log \left (\CM_{i, i \mid s_i \setminus \{ i \} } \right ) &=
...
minimize KL divergence end-to-end, as we proposed for sparsity selection in \cref{sec:select}.
...
selection algorithm described in \cref{sec:select} to select the
...
\begin{figure}[t]
  ...
  \caption{
    We perform Gaussian process regression on the SARCOS dataset by sparse
...
OCO-2 is a polar-orbiting satellite designed to measure atmospheric carbon dioxide.
...
\begin{figure}[t]
  ...
  \caption{
    We perform Gaussian process regression on the OCO-2 dataset.
...
MK was partially supported by National Science Foundation (NSF) Grant DMS--1953005.
