...
\begin{abstract}
  ...
  conjugate gradient, improving over \( k \)-nearest neighbors selection.
\end{abstract}

% REQUIRED
\begin{keywords}
  Gaussian process, Cholesky factorization, Vecchia approximation, factorized sparse approximate inverse, orthogonal matching pursuit, optimal experimental design
\end{keywords}
...
the previously ordered variables \cite{vecchia1988estimation}.
...
analyze the sparsity set and identify opportunities to group similar points into supernodes \cite{stein2004approximating, ferronato2015novel,
...
\times D} \) and the measurements \( y_i = f(\vec{x}_i) \) at those points are collected in the
...
a series of independent regression problems: each to predict the \( i
...
We now consider extensions of the single-column
...
Proper ``partial'' selection accounting for this matches
the asymptotic time and space complexities of the multiple-target algorithm
by storing a partial Cholesky factor. Details are provided in
...
\begin{figure}[t]
  ...
  \caption{%
    We perform Gaussian process regression on the SARCOS dataset by
    sparse Cholesky factorization of the joint covariance matrix.
...
in \emph{every} numerical experiment we tried over a wide
variety of geometries and tasks, a testament to its robustness.
...
It is difficult to compare the total computational time of the methods,
because increasing the number of iterations (e.g., by demanding better
tolerance or using matrices with worse condition numbers) will prefer more
...
Cholesky factor is mostly equivalent to IOU.
