...
\) ,\( f(X) \sim \mathcal{N}(\vec{\mu}, \CM) \), where \( \vec{\mu}_i =
...
\(
  \CM =
  \begin{pmatrix}
    \CM_{\text{Tr}, \text{Tr}} & \CM_{\text{Tr}, \text{Pr}} \\
    \CM_{\text{Pr}, \text{Tr}} & \CM_{\text{Pr}, \text{Pr}}
  \end{pmatrix}
\)
where \( \CM_{\text{Tr}, \text{Tr}}, \CM_{\text{Pr}, \text{Pr}},
\CM_{\text{Tr}, \text{Pr}}, \CM_{\text{Pr}, \text{Tr}} \) are the
...
\begin{align}
  ...
  \E[\vec{y}_\text{Pr} \mid \vec{y}_\text{Tr}] &=
    \vec{\mu}_\text{Pr} +
    \CM_{\text{Pr}, \text{Tr}} \CM_{\text{Tr}, \text{Tr}}^{-1}
  ...
  \CM_{I, J \mid K_{1 \dots n}} &=
    \CM_{I, J \mid K_{1 \dots n - 1}} -
    \CM_{I, K_n \mid K_{1 \dots n - 1}}
    \CM_{K_n, K_n \mid K_{1 \dots n - 1}}^{-1}
    \CM_{K_n, J \mid K_{1 \dots n - 1}}
\end{align}
...
training covariance matrix \( \CM_{\text{Tr}, \text{Tr}} \), which costs
\( \BigO(N^3) \), where \( N \) is the number of training points. This
...
cost \( \BigO(s^3) \), which could be substantially cheaper if \( s \)
...
\begin{align}
  \label{eq:info}
  \MI[\vec{y}_\text{Pr};\vec{y}_\text{Tr}] &= \entropy[\vec{y}_\text{Pr}] -
...
\begin{align}
  \textcolor{orange}{\entropy[\vec{y}_\text{Pr}]} &=
    ...
    \textcolor{rust}{\MI[\vec{y}_\text{Pr};\vec{y}_\text{Tr}]} \\
...
\begin{align}
  ...
  \label{eq:cond_select}
                          &= \CM_{:, : \mid I} -
    \frac{\CM_{:, k \mid I} \CM_{:, k \mid I}^{\top}}{\CM_{kk \mid I}}
\end{align}

We see that conditioning on a new entry is a rank-one update on the current
covariance matrix \( \CM_{\mid I} \), given by the vector \( \vec{u} =
\frac{\CM_{:, k \mid I}}{\sqrt{\CM_{kk \mid I}}} \). Thus, the amount
...
\begin{align}
  ...
  \frac{\CM_{k, n + 1 \mid I}^2}{\CM_{kk \mid I}}
...
with the initial values given by \( \CM_{kk} \) and \( \CM_{nk} \) and
...
strategies. The direct method is to keep track of \( \CM_{I, I}^{-1} \),
...
\BigO(s^2) \), see \cref{app:prec_insert}. Once \( \CM_{I, I}^{-1}
...
\), costing \( \BigO(N s^2 + s^3) = \BigO(N s^2) \) overall.
...
\begin{align}
  ...
  \begin{pmatrix}
    \chol(\CM_{1, 1}) & 0 \\
    \CM_{2, 1} \chol(\CM_{1, 1})^{-\top} & \chol(
      \CM_{2, 2} - \CM_{2, 1} \CM_{1, 1}^{-1} \CM_{1, 2}
    )
  \end{pmatrix}
\end{align}
Here \( \textcolor{darkorange}{\CM_{2, 1} \CM_{1, 1}^{-1}} \)
...
it costs \( \BigO(Ns) \) to compute the next column of the
Cholesky factorization, for a total cost of \( \BigO(Ns^2) \),
...
algorithm uses \( \BigO(s^2) \) space to store the precision while the
Cholesky factorization uses \( \BigO(N s) \) to store the first \( s \)
columns of the Cholesky factorization of \( \CM \), which is always more
memory than the precision (\( N > s \)). Both algorithms use an additional \(
\BigO(N) \) space to store the conditional variances and covariances.
...
\cref{eq:cond_mean} and \cref{eq:cond_cov} in time \( \BigO(s^3)
...
selected points, or \( \logdet(\CM_{\text{Pr}, \text{Pr} \mid I}) \). This
...
\begin{align}
  ...
  \logdet
  \left (
    \CM_{\text{Pr}, \text{Pr} \mid I \cup \{ k \}}
  \right ) &= \logdet
    \left (
      \CM_{\text{Pr}, \text{Pr} \mid I} -
      \frac{\CM_{\text{Pr}, k \mid I}
            \CM_{\text{Pr}, k \mid I}^{\top}
           }{\CM_{kk \mid I}}
    \right ) \\
  \nonumber
           &= \logdet \left ( \CM_{\text{Pr}, \text{Pr} \mid I} \right ) +
    \log
    \left (
      1 -
      \frac{\CM_{\text{Pr}, k \mid I}^{\top}
            \CM_{\text{Pr}, \text{Pr} \mid I}^{-1}
            \CM_{\text{Pr}, k \mid I}
           }{\CM_{kk \mid I}}
    \right )
  ...
  \shortintertext{By the quotient rule, we combine the conditioning:}
    ...
    \log
    \left (
      \frac{\CM_{kk \mid I, \text{Pr}}}{\CM_{kk \mid I}}
    \right )
\end{align}
...
m \) prediction points it costs \( \BigO(m^3) \) to compute \(
\CM_{\text{Pr}, \text{Pr}}^{-1} \) and then \( \BigO(N m^2) \)
to compute \( \CM_{kk \mid \text{Pr}} \) for the \( N \) candidates \(
k \). For each of the \( s \) rounds of selecting candidates, it costs
\( s^2 \) and \( m^2 \) to update the precisions \( \CM_{I, I}^{-1}
\) and \( \CM_{\text{Pr}, \text{Pr}}^{-1} \) respectively, where the
details of efficiently updating \( \CM_{\text{Pr}, \text{Pr}}^{-1}
\) after the rank-one update in \cref{eq:obj_gp_mult} are given in
\cref{app:prec_cond}. Given the precisions, \( \vec{u} = \frac{\CM_{:,
k \mid I}}{\sqrt{\CM_{kk \mid I}}} \) and \( \vec{u}_\text{Pr} =
\frac{\CM_{:, k \mid I, \text{Pr}}}{\sqrt{\CM_{kk \mid I, \text{Pr}}}}
\) are computed as usual according to \cref{eq:cond_cov} in time \( Ns
\) and \( Nm \). Finally, for each candidate \( j \), the conditional
variance \( \CM_{jj \mid I} \) is updated by subtracting \( u_j^2 \),
the conditional covariance \( \CM_{\text{Pr}, k \mid I} \) is updated
for each prediction point index \( c \) each by subtracting \( u_j u_c
\), and the conditional variance \( \CM_{jj \mid I, \text{Pr}} \) is
...
\) and \( \vec{u}_\text{Pr} \) and both conditional variances \( \CM_{jj
\mid I} \) and \( \CM_{jj \mid I, \text{Pr}} \) can be computed as above.
The conditional covariances do not need to be computed. Over \( s \) rounds
the total time complexity is \( \BigO((N + m) m^2 + s(N + m)(m + s)) \)
...
\BigO(s^2 + m^2) \) memory to store both precisions, as well as \(
\BigO(N m) \) memory to store the conditional covariances. The Cholesky
algorithm, on the othe rhand, requires \( \BigO((n + m)(m + s)) \) to
...
to \( \BigO(N s + N m + m^2) \). Comparing the memory usage, they are
...
\( \CM = L L^{\top} \). \Stodo{justify importance/downstream applications
of Cholesky factorization}. This can be done in \( \BigO(N^3) \) with
...
also took \( \BigO(N^3) \) to invert the covariance matrix \( \CM
...
columns in the group, \( \BigO(m s^2) \) (\( N > s > m \)). Thus, the
overall complexity is \( \BigO(\frac{N}{m} (N (s - m)^2 + N m^2 + m^3 +
s^3)) \), which simplifies to \( \BigO(\frac{N^2 s^2}{m}) \) by making
use of the bound that \( (s - m)^2 = \BigO(s^2 + m^2) \).
...
We have the matrix \( \CM_{I, I}^{-1} \) corresponding to the precision of
the selected entries, and wish to take into account the addition of a new entry
\( k \) into \( I \). That is, we wish to compute \( \CM_{I', I'}^{-1} \)
for \( I' = I \cup \{ k \} \), which in effect adds a new row and column to \(
\CM_{I, I}^{-1} \). In order to invert the new matrix efficiently, we can
...
\begin{align}
  ...
  \begin{pmatrix}
    I & 0 \\
    \textcolor{darkorange}{\CM_{2, 1} \CM_{1, 1}^{-1}} & I
  \end{pmatrix}
  \begin{pmatrix}
    \CM_{1, 1} & 0 \\
    0 & \textcolor{lightblue}{
  ...
  \end{pmatrix}
  \begin{pmatrix}
    I & \textcolor{darkorange}{\CM_{1, 1}^{-1} \CM_{1, 2}} \\
    0 & I
  \end{pmatrix}
  \shortintertext{For notational convenience, we denote the Schur
    complement \textcolor{lightblue}{\( \CM_{2, 2} - \CM_{2, 1}
    \CM_{1, 1}^{-1} \CM_{1, 2} \)} as \textcolor{lightblue}{\(
    \CM_{2, 2 \mid 1} \)}. Inverting both sides of the equation,}
  \CM^{-1} &=
  \begin{pmatrix}
    I & -\textcolor{darkorange}{\CM_{1, 1}^{-1} \CM_{1, 2}} \\
    0 & I
  ...
  \shortintertext{In the context of adding a new entry to the matrix,
    \( \CM_{1, 1} = \CM_{I, I} \), \( \CM_{1, 2} = \CM_{I,
    k} \), and \( \CM_{2, 2} = \CM_{kk} \). Also note that \(
    \textcolor{lightblue}{\CM_{kk \mid I}^{-1}} \) is the inverse
    of the variance of \( k \) conditional on the entries in \( I \),
    which has already been computed in \cref{alg:gp_select}. If we let
    \( \vec{v} = \textcolor{darkorange}{\CM_{I, I}^{-1} \CM_{I,
    k}} \), then we can write the update as:}
  &=
  \begin{pmatrix}
    \CM_{I, I}^{-1} + \CM_{kk \mid I}^{-1} \vec{v} \vec{v}^T &
    -\CM_{kk \mid I}^{-1} \vec{v} \\
    -\CM_{kk \mid I}^{-1} \vec{v}^{\top} & \CM_{kk \mid I}^{-1} \\
  \end{pmatrix}
\end{align}
which is precisely the update in line 13 of \cref{alg:gp_select}. Note that
the update is a rank-one update to \( \CM_{1, 1}^{-1} \), which can
be computed in \( \BigO(\lvert I \rvert^2) = \BigO(s^2) \).
...
We have the matrix \( \CM_{\text{Pr}, \text{Pr} \mid I}^{-1} \), or
...
\( \CM_{\text{Pr}, \text{Pr} \mid I \cup \{ k \}}^{-1} \) which is a rank-one
...
\begin{align}
  ...
  \right )^{-1} &=
    ...
    \CM_{2, 2 \mid 1}^{-1}
    \left (\CM_{2, 1} \CM_{1, 1}^{-1} \right )
  \shortintertext{Letting \( \vec{u} = \CM_{1, 2} \) and \( \vec{v}
    = \CM_{1, 1}^{-1} \CM_{1, 2} = \CM_{1, 1}^{-1} \vec{u} \),}
  ...
  \shortintertext{So we see that a rank-one update to \( \CM_{1, 1} \)
    then inverting is a rank-one update to \( \CM_{1, 1}^{-1} \). In our
    context, \( \CM_{1, 1} = \CM_{\text{Pr}, \text{Pr} \mid I}, \vec{u} =
    \CM_{\text{Pr}, k \mid I}, \CM_{2, 2} = \CM_{kk \mid I} \) so \(
    \CM_{2, 2 \mid 1}^{-1} = \CM_{kk \mid \text{Pr}, I}^{-1} \) (this can
    ...
    computed according to definition as \( \CM_{\text{Pr}, \text{Pr} \mid
    I}^{-1} \vec{u} \). Thus, we can write the update as}
  \left ( \CM_{\text{Pr}, \text{Pr} \mid I} -
    \frac{\CM_{\text{Pr}, k \mid I} \CM_{\text{Pr}, k \mid I}^{\top}}
    {\CM_{kk \mid I}}
  \right )^{-1} &=
    \CM_{1, 1}^{-1} +
    \CM_{kk \mid \text{Pr}, I}^{-1} \vec{v} \vec{v}^{\top}
\end{align}
which is the update in line 18 of \cref{alg:gp_select_mult}.
Since the update is a rank-one update, it can be computed in \(
\BigO(\lvert \text{Pr} \rvert^2) = \BigO(m^2) \).
...
\( \log(\CM_{kk \mid I, \text{Pr}}) \) for some candidate \( k \)
...
For a single prediction point, the objective is \( \frac{\CM_{k, \text{Pr}
...
selected, that is, \( \CM_{\text{Pr}, \text{Pr} \mid I} - \CM_{\text{Pr},
\text{Pr} \mid I, k} = \frac{\CM_{k, \text{Pr} \mid I}^2}{\CM_{kk \mid
...
\begin{align}
  \min \Delta &= \min \left [
    \log(\CM_{\text{Pr}, \text{Pr} \mid I, k}) -
  \log(\CM_{\text{Pr}, \text{Pr} \mid I}) \right ] \\
              &= \min \frac{\CM_{\text{Pr}, \text{Pr} \mid I, k}}
                           {\CM_{\text{Pr}, \text{Pr} \mid I}} \\
              &= \min \frac{\CM_{\text{Pr}, \text{Pr} \mid I}
              - \frac{\CM_{k, \text{Pr} \mid I}^2}{\CM_{kk \mid I}}}
                           {\CM_{\text{Pr}, \text{Pr} \mid I}} \\
              &= \min \left [ 1 -
                \frac{\CM_{k, \text{Pr} \mid I}^2}
                     {\CM_{kk \mid I} \CM_{\text{Pr}, \text{Pr} \mid I}}
                 \right ] \\
  \label{eq:global_obj}
              &= \max
                \frac{\CM_{k, \text{Pr} \mid I}^2}
...
\begin{align*}
  \MI(\text{Pr}, I \cup \{ x_2 \}) - \MI(I) &\overset{?}{\geq}
    \MI(\text{Pr}, I \cup \{ x_1, x_2 \}) - \MI(\text{Pr}, I \cup \{ x_1 \})
  ...
  \shortintertext{Since this is the objective \cref{eq:obj_gp_mult}
    (with additional log),}
  \frac{\CM_{x_2, x_2 \mid I}}{\CM_{x_2, x_2 \mid I, \text{Pr}}}
    &\overset{?}{\geq}
    \frac{\CM_{x_2, x_2 \mid I} -
          \frac{\CM_{x_1, x_2 \mid I}^2}{\CM_{x_1, x_1 \mid I}}}
         {\CM_{x_2, x_2 \mid I, \text{Pr}} -
         \frac{\CM_{x_1, x_2 \mid I, \text{Pr}}^2}
              {\CM_{x_1, x_1 \mid I, \text{Pr}}}}
