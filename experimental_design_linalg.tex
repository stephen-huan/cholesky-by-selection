...
rule of Schur complements \cref{eq:quotient_rule}.
...
% TODO: justify why (single point selection
% independently, possibly more efficient, ``two birds with one stone'')
...
We can now generalize the multiple-target algorithm to track the log
...
\begin{align}
  ...
  \shortintertext{
    where unlike \cref{eq:chol_partial}, we are using a
...
determinant objective \cref{eq:partial_kl} after adding a candidate.
...
time complexity of the multiple-target algorithm.
